...
- 通知区域右键菜单可选择开机启动；该选项默认关闭，启用后使用后台模式启动。
...
启动 `KeyPulse.exe` 后即开始统计。标题栏右侧的短横按钮会隐藏窗口，但程序仍在后台运行；单击标题栏状态可暂停或继续统计。右键通知区域图标可以重新打开窗口、切换开机启动或真正退出程序。开机启动默认关闭。

使用 `KeyPulse.exe --background` 可直接在后台启动，只显示通知区域图标而不弹出主窗口。开机启动项使用该参数；普通双击启动仍会显示主窗口。
...
- `src/command_line.cpp` / `src/command_line.h`：启动参数识别。
- `src/command_line_tests.cpp`：后台启动参数测试。
...
- `src/startup.cpp` / `src/startup.h`：当前用户开机启动状态查询与显式启用/停用。
